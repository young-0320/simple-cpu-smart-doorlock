...
module top_cpu (
    ...
    // =======================================================
    // 기본 원칙:
    // - LOADI / ADDI / CMPI : imm 사용
    // - SHL / SHR           : addr 값 자체를 shift amount로 사용
    // - AND                 : BRAM[addr] 사용
    // - ADD / SUB / CMP     : BRAM[addr] 사용

    assign alu_operand =
        (is_alu_imm) ? {4'b0000, imm} :
        (is_ext && ((alu_ctrl == `ALU_SHL) || (alu_ctrl == `ALU_SHR))) ? {20'd0, addr} :
        bram_rdata;
    ...
    always @(*) begin
        ...
        case (state)
            ...
            `ST_EXECUTE: begin
                // LOAD, STORE, ADD, SUB, CMP는 BRAM[addr] 필요
                // EXT 중 AND도 BRAM[addr] 필요
                // EXT 중 SHL/SHR은 addr 값 자체를 shift amount로 쓰므로 BRAM read 불필요
                if (is_load || is_store || is_alu_mem ||
                    (is_ext && (alu_ctrl == `ALU_AND))) begin
                    bram_addr = addr;
                end
                else begin
                    bram_addr = pc_out;
    ...
    always @(*) begin
        ...
        if (clk_enable && (state == `ST_INCREMENT)) begin
            ...
            else if (is_alu_imm || is_alu_mem || is_ext) begin
