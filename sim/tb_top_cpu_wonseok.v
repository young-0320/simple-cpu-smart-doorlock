...
//
// 추가 검증:
// opcode 1111 A-type 확장 명령어
// SHL x  : ACC <= ACC << x[4:0]
// SHR x  : ACC <= ACC >> x[4:0]
// AND a  : ACC <= ACC & BRAM[a]
...
module tb_top_cpu_wonseok;
    ...
    initial begin
        ...
        // ---------------------------------------------------
        // 32~41: EXT A-type 테스트
        // SHL x  : ACC <= ACC << x[4:0]
        // SHR x  : ACC <= ACC >> x[4:0]
        // AND a  : ACC <= ACC & BRAM[a]
        // ---------------------------------------------------
        memory[32] = 32'h50000001; // LOADI 1      -> ACC = 1
        memory[33] = 32'hF0000004; // SHL 4        -> ACC = 1 << 4 = 16
        memory[34] = 32'h10000070; // STORE 112    -> memory[112] = 16

        memory[35] = 32'hF1000004; // SHR 4        -> ACC = 16 >> 4 = 1
        memory[36] = 32'h10000071; // STORE 113    -> memory[113] = 1

        memory[37] = 32'h5000000F; // LOADI 15     -> ACC = 15
        memory[38] = 32'h10000072; // STORE 114    -> memory[114] = 15

        memory[39] = 32'h5000000A; // LOADI 10     -> ACC = 10
        memory[40] = 32'hF2000072; // AND 114      -> ACC = 10 & memory = 10
        memory[41] = 32'h10000073; // STORE 115    -> memory[115] = 10

        // ---------------------------------------------------
        // 42~48: ADDI + CMPI + OUT + NOP 테스트
        // 최종 ACC = 7, ZERO_FLAG = 1, OUT = 7
        // ---------------------------------------------------
        memory[42] = 32'h50000000; // LOADI 0      -> ACC = 0
        memory[43] = 32'h60000007; // ADDI 7       -> ACC = 7
        memory[44] = 32'h1000006A; // STORE 106    -> memory[106] = 7
        memory[45] = 32'h70000007; // CMPI 7       -> ZERO_FLAG = 1
        memory[46] = 32'hC0000000; // OUT 0        -> out_port = 7
        memory[47] = 32'hB0000000; // NOP
        memory[48] = 32'h8000002F; // JMP 47       -> 마지막 대기 루프
        ...
        // 프로그램 실행 대기
        // 약 49개 instruction * 4 cycle * 20ns = 약 3920ns 이상 필요
        #5000;
        ...
        $display("----------------------------------------");
        $display("memory[112] SHL result      Expected 16 : %d", memory[112]);
        $display("memory[113] SHR result      Expected 1  : %d", memory[113]);
        $display("memory[114] AND mask        Expected 15 : %d", memory[114]);
        $display("memory[115] AND result      Expected 10 : %d", memory[115]);
        $display("----------------------------------------");
